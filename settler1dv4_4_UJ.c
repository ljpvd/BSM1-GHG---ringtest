 ...
 * TEMPMODEL used to define different temperature approaches
 * Extended from BSM2_standard by using two autotrophic organism,
 * NO2, NO, N2O, N2 for full GHG BSM2 implementation
 * Copyright: Ulf Jeppsson, IEA, Lund University, Lund, Sweden
 */

#define S_FUNCTION_NAME settler1dv4_4_UJ
...
{
    ssSetNumContStates(    S, 160); /* number of continuous states           van 80 naar 150 10x SNO2; 10x SNO; 10x SN2O; 10x SN2; 10xTEMP, 10x SD1; 10x SD2; 10x SD3*/
    ssSetNumDiscStates(    S, 0);   /* number of discrete states             */
    ssSetNumInputs(        S, 28);  /* number of inputs                      */ /* 26 statevariables and Qr, Qw*/
    ssSetNumOutputs(       S, 215); /* number of outputs                     */ /* 26 x underflow, 1x Qw, 26x overflow, 10x TSS, gamma, gamma_eff, 140x soluble variables */
...
static void mdlOutputs(double *y, double *x, double *u, SimStruct *S, int tid)
{
  ...
  if (modeltype < 0.5) {
     ...
     y[21]=x[120];      /* Dummy soluble */
     y[22]=x[130];      /* Dummy soluble */
     y[23]=x[140];      /* Dummy soluble */
     y[24]=u[24]*gamma; /* Dummy particulate */
     y[25]=u[25]*gamma; /* Dummy particulate */    
     ...
     y[32]=u[5]*gamma_eff;
     y[33]=u[6]*gamma_eff;
     y[34]=u[7]; /* use oxygen in effluent flow */
     y[35]=u[8];
     y[36]=u[9];
     y[37]=u[10];
     y[38]=u[11]*gamma_eff;
...
double v0_max, v0, r_h, r_p, f_ns, X_t, area, h, feedlayer, volume, modeltype, tempmodel;
...
if (tempmodel < 0.5) {         
   ...
   for (i = 150; i < 160; i++) {
      if (i < (feedlayer-1+150-eps))
         dx[i] = (-v_up*x[i]+v_up*x[i+1])/h;
      else if (i > (feedlayer+150-eps))
         dx[i] = (v_dn*x[i-1]-v_dn*x[i])/h;
      else
